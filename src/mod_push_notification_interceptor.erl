...
start(Host, Opts) ->
    ...
    % },
    {ok, Connection} =
        amqp_connection:start(#amqp_params_network{username = list_to_binary(RabbitUser),
...
-spec send_message_to_rb({stanza(), c2s_state()}) -> {stanza(), c2s_state()}.
send_message_to_rb({#message{from = From,
                             to = To,
                             type = Type,
                             sub_els = SubEls} =
                        Pkt,
    ...
    EncryptedMessage =
        try lists:nth(1, Body) of
            {_, _, EncryptedMessageFun} ->
                EncryptedMessageFun
        catch
            _:_ ->
                <<"">>
    ...
    SubscriptionItem =
        lists:filter(fun(Roster) -> element(1, Roster#roster.jid) == ToFind end, Items),
    % ?INFO_MSG("Subscription item is ~p", [SubscriptionItem]),
    % Take subscription value
    Subscription =
        case SubscriptionItem of
            [Item] ->
    ...
    IsToUserOffline =
        case ejabberd_sm:get_user_present_resources(To#jid.luser, To#jid.lserver) of
            [] ->
                true;
            _ ->
                false
